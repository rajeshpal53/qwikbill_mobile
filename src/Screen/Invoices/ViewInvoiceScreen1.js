...
//   useEffect(() => {
//     setPage(1); // Reset page to 1 whenever filter changes (selected, sortBy)
//     fetchInvoices(1); // Fetch new data for page 1
//   }, [selected, sortBy]);

//   useEffect(() => {
//     if (page > 1) fetchInvoices(page); // Trigger pagination when page changes
//   }, [page]);
...
//     if (sortBy === "datewise")
//       api += `&startDate=${formatDate(date.startDate)}&endDate=${formatDate(date.endDate)}`;
...
//     if (isLoading) return; // Prevent multiple fetches while already loading
//     setIsLoading(true);
//     try {
...
//         `invoice/searchInvoices?searchTerm=${query}`,
//         {
...
//               setsearchmodal={setModalVisible}
...
//         data={searchQuery ? searchedData : invoices}
//         renderItem={({ item }) => (
